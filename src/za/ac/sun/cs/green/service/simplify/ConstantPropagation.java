...
public class ConstantPropagation extends BasicService {
	...
	private static Map<Expression, Expression> replica;
	private static Map<Expression, Boolean> boolMap;
	...
	public Set<Instance> processRequest(Instance instance) {
		...
		if (result == null) {
			...
			replica = new HashMap<Expression, Expression>();
			boolMap = new HashMap<Expression, Boolean>();
	...
	public Expression canonize(Expression expression,
		...
		try {
			...
			if (prop != null) {
				prop = new Renamer(map,
						orderingVisitor.getVariableSet()).rename(prop);
			}
			CanonizationVisitor canonizationVisitor = new CanonizationVisitor();
			prop.accept(canonizationVisitor);
	...
	private static class OrderingVisitor extends Visitor {
		...
		public void postVisit(Operation operation) throws VisitorException {
			...
			if (nop != null) {
				...
				if ((l instanceof IntVariable) && (r instanceof IntConstant) && (nop == Operation.Operator.EQ)) {
					replica.put(l, r);
					boolMap.put(l, false);



				} else if ((r instanceof IntVariable) && (l instanceof IntConstant) && (nop == Operation.Operator.EQ)) {
					replica.put(r, l);
					boolMap.put(r, false);
				...
				if ((r instanceof IntVariable)
					...
					Operation tempPush1 = new Operation(nop, l, r);

					stack.push(tempPush1);
				} else if ((r instanceof IntVariable)
						&& (l instanceof IntConstant)) {

					Operation tempPush = new Operation(nop, l, r);
			...
			if (!linearInteger) {
				...
				for (Expression e : newConjuncts) {
					...
					if (o.getOperator() == Operation.Operator.GT) {
						e = new Operation(Operation.Operator.GT, scale(-1, o.getOperand(0)), o.getOperand(1));
					} else if (o.getOperator() == Operation.Operator.GE) {
						e = new Operation(Operation.Operator.GE, scale(-1, o.getOperand(0)), o.getOperand(1));
					}
					o = (Operation) e;
					if (o.getOperator() == Operation.Operator.GT) {
						e = new Operation(Operation.Operator.GT, merge(o.getOperand(0), new IntConstant(0)), o.getOperand(1));
					} else if (o.getOperator() == Operation.Operator.LT) {
						e = new Operation(Operation.Operator.LT, merge(o.getOperand(0), new IntConstant(0)), o.getOperand(1));
					}
					if (c == null) {
						c = e;
					} else {
		...
		public void postVisit(Operation operation) throws VisitorException {
			...
			switch (op) {
				...
				case GE:
					...
					Expression e = merge(scale(-1, temp1), temp2);
	...
	private static class Renamer extends Visitor {
		...
		public void postVisit(IntVariable variable) {
				...
				Expression temp = replica.get(variable);
				if (temp != null) {
					IntConstant tempC = new IntConstant(Integer.parseInt(temp.toString()));
					stack.push(tempC);


				} else {

						Variable v = new IntVariable(variable.toString(), variable.getLowerBound(),
		...
		public void postVisit(Operation operation) {
			...
			if ((count == 2) && (operands[0] instanceof IntConstant) && (operands[1] instanceof IntConstant) &&( replica.containsValue(operands[1]) ||replica.containsValue(operands[0]) ) && operation.getOperator() == Operation.Operator.EQ ) {
				if (replica.containsValue(operands[0])) {
					Expression arr[] = new Expression[2];
					Iterator run = replica.keySet().iterator();
					while (run.hasNext()) {
						Object tempEx = run.next();
						if (replica.get(tempEx).compareTo(operands[0]) == 0 && boolMap.get(tempEx).equals(false) ) {
							arr[0] = new IntVariable(tempEx.toString(), 0, 99);
							arr[1] = operands[1];
							boolMap.put((Expression)tempEx, true);

							break;
						}
					}

					stack.push(new Operation(operation.getOperator(), arr));
					return;
				} else if (replica.containsValue(operands[1])) {
					Expression arr[] = new Expression[2];
					Iterator run = replica.keySet().iterator();
					while (run.hasNext()) {
						Object tempEx = run.next();
						if (replica.get(tempEx).compareTo(operands[1]) == 0 && boolMap.get(tempEx).equals(false) ) {
							arr[1] = new IntVariable(tempEx.toString(), 0, 99);
							arr[0] = operands[0];
							boolMap.remove(tempEx);
							boolMap.put((Expression)tempEx, true);
				...
				Operation temp12 = new Operation(operation.getOperator(), operands);
				stack.push(temp12);
			}
